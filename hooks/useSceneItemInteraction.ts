...
export function useSceneItemInteraction(
  ...
  function handleClick(e: ThreeEvent<MouseEvent>) {
    if (canvasTool === 'look' || canvasTool === 'pan') return; // look/grab: don't select, let the camera move
    e.stopPropagation();
    if (isLocked) return;
    if (isEditMode) {
  ...
  function handlePointerDown(e: ThreeEvent<PointerEvent>) {
    ...
    if (canvasTool === 'look' || canvasTool === 'pan') return; // look/grab: let camera-controls move the view
    e.stopPropagation();
    ...
    setIsHovered(true);
    if (!isEditMode || canvasTool === 'look' || canvasTool === 'pan') return;
  ...
  const showHoverRing = isEditMode && isHovered && !isSelected && canvasTool !== 'look' && canvasTool !== 'pan' && !isDragActive;
